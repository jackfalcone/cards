...
class CardService implements CardServiceInterface
{
    ...
    public function fetchAndSaveCards(string $setCode): Collection|false
    {
        if (!$this->cardsExistInDb($setCode)) {
            try {
                $initialUrl = "https://api.scryfall.com/cards/search?q=set:{$setCode}";
                $this->fetchAndSaveCardsFromUrl($initialUrl);
            } catch (\Exception $e) {
    ...
    /**
     * Recursive function to fetch and save cards from a given URL.
     *
     * @param string $url
     * @return void
     */
    private function fetchAndSaveCardsFromUrl(string $url): void
    {
        try {
            ...
            if ($response->successful()) {
                ...
                if (isset($data['next_page'])) {
                    $this->fetchAndSaveCardsFromUrl($data['next_page']);
                }
            } else {
                Log::error('Failed to fetch cards from Scryfall', ['url' => $url, 'response' => $response->body()]);
            }
        } catch (\Exception $e) {
            Log::error('Error fetching cards from URL', ['url' => $url, 'message' => $e->getMessage()]);
        }
    }

    /**
     * Save card data to the database and dispatch jobs to save images.
     *
     * @param array $cardData
     * @return void
     */
    private function saveCardData(array $cardData): void
    {
        $data = [
